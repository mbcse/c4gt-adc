...
const AdminAnalyticsDashboard: React.FC = () => {
  ...
  const [completionDimension, setCompletionDimension] = useState<'grade' | 'school' | 'district'>('grade');
  const [quizDimension, setQuizDimension] = useState<'grade' | 'school' | 'district' | 'course'>('grade');
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              An overview of student engagement and performance metrics.
            </p>
          </div>
          <div className="mt-4 md:mt-0 flex items-center space-x-2">
            ...
            >
             ...
             {/* Download button can be wired up later */}
  ...
  {activeTab === 'overview' && dashboardData && (
              ...
              {dashboardData.courseCompletionRates.overall && (
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    <Pie
                      data={[
                        { name: 'Completed Enrollments', value: dashboardData.courseCompletionRates.overall.completedCourses },
                        { name: 'In-Progress Enrollments', value: dashboardData.courseCompletionRates.overall.totalEnrollments - dashboardData.courseCompletionRates.overall.completedCourses }
                      ]}
                      cx="50%"
                      cy="50%"
                      innerRadius={60}
          ...
          {activeTab === 'consistency' && consistencyRates && (
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              ...
              {/* Consistency by dimension tables */}
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <table className="min-w-full text-sm">
                      <thead>
                        <tr className="border-b border-gray-200">
                          ...
                          <th className="px-3 py-2 text-center font-medium text-gray-700">High</th>
                        </tr>
                      </thead>
                      <tbody>
                        {consistencyRates.byGrade.map((item, index) => (
                          <tr key={index} className="border-b border-gray-100">
                            <td className="px-3 py-2 font-medium text-gray-900">Grade {item.grade}</td>
                            <td className="px-3 py-2 text-center">
                              <span className={`font-medium ${
                                item.avgConsistencyRate >= 70 ? 'text-green-600' :
                                item.avgConsistencyRate >= 50 ? 'text-yellow-600' : 'text-red-600'
                              }`}>
                                {item.avgConsistencyRate}%
                              </span>
                            </td>
                            <td className="px-3 py-2 text-center text-gray-700">{item.highConsistencyStudents}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-full text-sm">
                      <thead>
                        <tr className="border-b border-gray-200">
                          <th className="px-3 py-2 text-left font-medium text-gray-700">School</th>
                          <th className="px-3 py-2 text-center font-medium text-gray-700">Avg %</th>
                          <th className="px-3 py-2 text-center font-medium text-gray-700">High</th>
                        </tr>
                      </thead>
                      <tbody>
                        {consistencyRates.bySchool.slice(0, 5).map((item, index) => (
                          <tr key={index} className="border-b border-gray-100">
                            <td className="px-3 py-2 font-medium text-gray-900 max-w-32 truncate">{item.school}</td>
                            <td className="px-3 py-2 text-center">
                              <span className={`font-medium ${
                                item.avgConsistencyRate >= 70 ? 'text-green-600' :
                                item.avgConsistencyRate >= 50 ? 'text-yellow-600' : 'text-red-600'
                ...
                <div>
                  <h4 className="font-semibold text-gray-900 mb-3">By District</h4>
                  <div className="overflow-x-auto">
                    <table className="min-w-full text-sm">
                      <thead>
                        <tr className="border-b border-gray-200">
                      ...
                      <tbody>
                        {consistencyRates.byDistrict.map((item, index) => (
                          <tr key={index} className="border-b border-gray-100">
                            <td className="px-3 py-2 font-medium text-gray-900 max-w-32 truncate">{item.district}</td>
                            <td className="px-3 py-2 text-center">
                              <span className={`font-medium ${
                                item.avgConsistencyRate >= 70 ? 'text-green-600' :
                                item.avgConsistencyRate >= 50 ? 'text-yellow-600' : 'text-red-600'
        ...
        {/* Individual Student Filter Info */}
